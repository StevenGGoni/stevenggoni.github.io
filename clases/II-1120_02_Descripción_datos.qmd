...
subtitle: "[Versión PDF](https://stevenggoni.github.io/clases/II-1120_02_Descripción_datos.pdf) <br> II-1120 Estadística para Ingeniería Industrial I"
date: today
format: revealjs
---
...
motivo_ord$Motivo <- factor(motivo_ord$Motivo,
                          levels = motivo_ord$Motivo)
...
-   $$\mu_w = \frac{\sum_{i=1}^{N} w_i \cdot x_i}{\sum_{i=1}^{N} w_i}$$
:::
:::::
...
## 4. Cuantiles {.bloques style="font-size: 0.92em;"}
...
-   Dividen el conjunto de datos en partes iguales. El procedimiento manual siempre inicia con ordenar los datos de menor a mayor (*en este sentido se sigue la misma lógica de cálculo que en la mediana, pues esta es un caso de los cuantiles*). Y tomar decisiones si la cantidad de datos es par o impar.
    ...
    -   **Mediana**: divide a los datos en 2 partes iguales El $P_{50}$ es igual al $Q_2$ que es igual a la mediana ($\tilde{x}$).
- La fórmula general son
  * $L = 1 + (n-1)\frac{k}{100}$, 
  * $L= \frac{k(n+1)}{100}$
  * Siendo $L$ la posición del percentil ($k$). Si $L$ es un entero, se toma ese dato, caso contrario, se interpola entre los datos vecinos.

## ¿Cómo se interpretan? {.bloques}

::::: columns
::: {.column width="60%" style="font-size: 0.95em;"}
-   Por ejemplo, para el siguiente conjunto de datos el $Q_1$ es 35 para la primera fórmula y 33 para la segunda.
    -   29, 31, 35, 39, 40, 43, 44, 44, 52
    - $L = 1 +(9-1)\frac{25}{100} = 1+8\cdot 0.25 = 3$
    * $L = \frac{25(9+1)}{100}=2.5$, 
        -   En Excel `QUARTILE.INC` es la primera fórmula y `QUARTILE.EXC` es la segunda.
-   [Interpretación]{.hi}:
    -   El 25 % de los datos se encuentran por debajo de 35.
-   Calcule e interprete otros cuantiles.
-   [Nota]{.hi}: es más importante la interpretación, que la metodología de cálculo, pues esta puede variar entre los software.
:::
...
- Se define técnicamente como el cuarto momento estandarizado esperado de una variable aleatoria $X$. 

  * Se basa en la desviación respecto a la media elevada a la cuarta potencia ($E[(X-\mu)^4] / \sigma^4$), lo que magnifica exponencialmente el impacto de las observaciones distantes.

- La interpretación correcta de la curtosis es en términos de la [extremidad de las colas]{.hi}: mide la propensión de una distribución a producir valores atípicos ([outliers]{.hi}), y **no** la "puntiagudez" ([peakedness]{.hi}) o forma del pico central.

  - Tradicionalmente se ha confundido con cuán escarpada o achatada se encuentra una distribución; sin embargo, esta interpretación es un mito estadístico desmentido (Westfall, 2014).
...
- Usamos la misma fórmula muestral que MS Excel:

    - Mide el [exceso de curtosis]{.underline}; es decir, la diferencia respecto a una distribución normal (tema que se abordará más adelante).

- $ku = \left[ \frac{n(n+1)}{(n-1)(n-2)(n-3)} \sum_{i=1}^{n} \left(\frac{x_i-\bar{x}}{s}\right)^4 \right] - \frac{3(n-1)^2}{(n-2)(n-3)}$

- $ku < 0$ [Platicúrtica]{.hi}:

  * Muestra una curtosis menor que la normal. Indica una baja propensión a producir valores atípicos.
  
- $ku = 0$ [Mesocúrtica]{.hi}: 

  * Presenta una curtosis idéntica a la de la distribución normal.
  
- $ku > 0$ [Leptocúrtica]{.hi}: 

  * Muestra una curtosis mayor que la normal. Indica una alta propensión a producir valores atípicos (colas pesadas).
...
* Su única interpretación libre de ambigüedad es en términos de la extremidad de las colas (tail extremity).

* Westfall demuestra matemáticamente que la porción de la curtosis que viene determinada por los datos del centro de la distribución (es decir, aquellos dentro del rango de una desviación estándar de la media, $\mu \pm \sigma$) es sumamente pequeña e insignificante en casi todas las distribuciones comunes.
...
<iframe 
src="https://stevenggoni.github.io/interactivos/curtosis-embded.html"
width="100%" 
height="700" 
...
histograma <- data.frame(Datos = c(73.8, 74.2, 75.1, 75.3, 75.5, 75.7, 
                                   76.5, 76.9, 77.1, 77.2, 77.2, 77.2))
...
|:---:|:---|:---|:---:|:---:|:---:|:---:|
| 1 | $LI_1=[Min$ | $LS_1=LI_1 + h[$ | Conteo | $\frac{f_{a1}}{n}$ | $f_{a1}$ | $\frac{F_{a1}}{n}$ |
| 2 | $[LI_2 = LS_1$ | $LS_2 = LI_2 + h[$ | … | $\frac{f_{a2}}{n}$ | $f_{a1}+f_{a2}$ | … |
| 3 | $[LI_3 = LS_2$ | $LS_3 = LI_3 + h[$ | … | … | … | … |
| 4 | $[LI_4 = LS_3$ | $LS_4 = LI_4 + h[$ | … | … | … | … |
| 5 | $[LI_5 = LS_4$ | $LS_5 = LI_5 + h = Max]$ | … | … | $f_{a1}+\dots+f_{a5}=n$ | $\frac{F_{a5}}{n}=1$ |
...
histograma %>%
  dplyr::mutate(Clase = cut(Datos, 
                            breaks = seq(min(Datos), max(Datos), by=0.68), 
...
df_bins <- histograma %>%
  dplyr::mutate(Clase = cut(Datos, 
                            breaks = seq(73.8, 77.2, by = 0.68), 
                            include.lowest = TRUE, 
                            right = FALSE)) %>%
  dplyr::count(Clase, .drop = FALSE) %>%
  dplyr::mutate(EjeX = c("[73.8-74.48)", "[74.48-75.16)", 
                         "[75.16-75.84)", "[75.84-76.52)", "[76.5-77.20]"))

plotly::plot_ly(data = df_bins,
                x = ~EjeX,
                y = ~n,
                type = "bar",
                marker = list(color = "#263247",
                              line = list(color = "#C18A00", width = 1.5)),
                height = 600,
                width = 850) %>%
...
* Tome en cuenta que `()` se interpreta como intervalo abierto y `[]` como cerrado. 
...
## Una variable cuantitativa y otra cualitativa {.bloques}
