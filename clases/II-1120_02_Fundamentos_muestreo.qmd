...
-   Cuando el método de recopilación de datos hace que los datos de la muestra reflejen incorrectamente la población. En términos simples, es la diferencia que existe entre un estimador ($\bar{x}$) y un parámetro ($\mu$).
    -   $|\bar{x}-\mu|$
...
-   Es un método de recolección de 𝑛 unidades en un conjunto de $N$ de tal modo que cada una de las $\binom{N}{n} = \frac{N!}{n!(N-n)!}$ muestras distintas tengan la misma oportunidad de ser elegidas.
...
* El cálculo de $n$ debe dar un resultado [ADIMENSIONAL]{.hi}, por lo que "las unidades" de $\sigma$ y $e$ deben estar en la misma dimensión. 
...
* [¿Qué es $e$?]{.hi} y [¿cómo escojo $\alpha$]{.hi}
  * Es el máximo error permitido por la persona investigadora que tiene un IMPACTO sobre el contexto en el que se enmarca el muestreo. Lo mismo ocurre con $\alpha$, depende del contexto. Puede encontrar más detalle en esta [infografía](https://stevenggoni.github.io/infografias/Comprendiendo%20el%20valor%20P.pdf){target="_blank"}.
...
* Todo tamaño de muestra se debe [redondear] a un número entero. La norma indica que este redondeo siempre debe ser hacia el [entero superior más próximo]{.hi}. 
