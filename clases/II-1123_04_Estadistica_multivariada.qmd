...
* Con base en esta, estime el error (también conocido como [residuos]{.hi}) y obtenga la suma de todos los residuos. 
  * $$e = y-\hat{y}$$
* En teoría este valor debe ser cero o aproximadamente cero. 
* De forma matricial, lo puede obtener de esta manera:
...
* Cuando se rechaza la hipótesis nula se dice que el [coeficiente es significativo]{.hi}.
* Recuerde además, que, para la media:
...
* Las variables *dummy* solucionan este problema, convirtiendo las 𝑛 categorías en $n-1$ variables cuantitativas.
...
* En este caso $\beta_1$ representa a la tela B. Entonces se puede decir que la cuando se use la tela B se espera una reducción en el [promedio]{.hi} de la resistencia de 2.71, pues el estimador es negativo. 
...
::: {.iframe-card}
<iframe src="diagramas/pasos_regresion.html" title="Pasos de una RLM"></iframe>
:::
...
::: {.iframe-card}
<iframe src="diagramas/datos+teoria.html" title="Pasos de una RLM"></iframe>
:::
...
* No en todas las ocasiones se le va a proveer un conjunto de datos para que haga la regresión, creer que es así es muu inocente. En muchas ocasiones es usted en su rol como persona ingeniera quien tiene que decidir cuántos y cuáles datos recoger.
...
::: {.iframe-card}
<iframe src="diagramas/especificacion.html" title="Pasos de una RLM"></iframe>
:::
...
* $$y_ij= \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_1 x_2 + e_{ij}$$
...
* Donde el objetivo es minimizar la respuesta y $x_1$ y $x_2$ son el número de cajas y la distancia recorrida, respectivamente. Se aclara que el min en la ecuación es de minutos, no de minizar. 

## Estimación de parámetros {.bloques}

::: {.iframe-card}
<iframe src="diagramas/estimacion.html" title="Pasos de una RLM"></iframe>
:::
...
* Hoy por hoy el método en “boga” (desde los 70’s, formalizados por John Nelder y Robert Wedderburn) para la estimación de parámetros es el de máxima verosimilitud (ML – Maximum Likelihood).
...
::: {.iframe-card}
<iframe src="diagramas/adecuacion.html" title="Pasos de una RLM"></iframe>
:::
...
# ¿Qué más puedo hacer sino se cumplen los supuestos? {.bloques}
...
::: {.iframe-card}
<iframe src="diagramas/validacion.html" title="Pasos de una RLM"></iframe>
:::
...
* La validez del modelo se puede determinar de varias formas: 
  ...
  * [Partición de datos]{.hi}, es deci rpartir los datos recolectados en dos partes. Se emplea una para hacer la estimación y la otra parte para validar que el modelo funcione.
...
::: {.iframe-card}
<iframe src="diagramas/uso_modelo.html" title="Pasos de una RLM"></iframe>
:::

## Interpretación de resultados {.bloques}

* Esta sección se refiere a como se interpretan correctamente los resultados obtenidos una vez [todo el proceso para obtener una regresión ha sido realizado]{.hi}. 
...
* Para ello miden el estrés hasta la rotura (variable cuantitativa dependiente) para dos tipos de templado (lento y rápido) y tres grosores de lámina (8mm, 16mm y 24 mm).
* Use un 95 % de confianza
