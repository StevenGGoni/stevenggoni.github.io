...
subtitle: "[Versión PDF](https://stevenggoni.github.io/clases/II-1123_04_Bondad_ajuste.pdf) <br> II-1123 Estadística para Ingeniería Industrial II"
date: today
format: revealjs
---
...
-   Describe qué tan bien se ajusta un conjunto de observaciones. Tales medidas, por lo general, describen o resumen la discrepancia entre los valores observados y los valores esperados en el modelo de estudio.
...
## Recuerde: Visualización {.bloques}
...
## Rutinas gráficas: Formalmente {.bloques}
...
plotly::ggplotly(g1, height = 600, width = 570) |> 
...
::: {.column width="20%" style="font-size: 0.90em;"}
...
-   Gráfico de barras (Variables discretas)
    -   Puede agregar puntos teóricos que le permitan evaluar el ajuste.
...
plotly::ggplotly(g2, height = 600, width = 570) |> 
  plotly::layout(paper_bgcolor = "#F1F5F5",
                 plot_bgcolor  = "#F1F5F5",
                 font = list(size = 30),
                 xaxis = list(
...
plotly::plot_ly(height = 670, width = 850) |>
...
-   También es relevante para saber qué tipo de prueba de hipótesis realizar.
...
pred_conf <- predict(fit, interval = "confidence")

plotly::plot_ly(height = 600, width = 650) |>
  plotly::add_ribbons(x = theoretical_ln,
                      ymin = pred_conf[, "lwr"], # Límite inferior del modelo
                      ymax = pred_conf[, "upr"], # Límite superior del modelo
                      fillcolor = "rgba(38,50,71,0.15)",
                      line = list(color = "transparent"),
                      showlegend = FALSE) |>
  ...
  plotly::layout(paper_bgcolor = "#F1F5F5",
                 ...
                 xaxis = list(
                   title = list(text = "Cuantiles teóricos Lognormal(-1.24, 1.29)",
                                font = list(size = 25)),
                   tickfont = list(size = 20)),
                 yaxis = list(title = list(text = "Datos Muestrales", # Etiqueta corregida
                                            font = list(size = 25)),
                              range = c(0, 5),
                              tickfont = list(size = 20)))
```
...
::::: columns
::: {.column width="50%"}
-   El valor P es una probabilidad que [mide la evidencia en contra de la hipótesis nula]{.hi}. [Cuanto más bajo sea este valor, más fuerte será la evidencia en contra]{.underline}.
...
-   Consulte esta [infografía](https://stevenggoni.github.io/infografias/Comprendiendo%20el%20valor%20P.pdf){target="_blank"}, si es necesario, para repasar estos conceptos.
:::
...
-   Es un test estadístico que compara las frecuencias observadas con las frecuencias esperadas según la distribución teórica propuesta.
    -   También conocido como prueba de Pearson.
...
-   Se define la prueba de hipótesis
    ...
    -   $H_o:$ Los datos observados provienen de la distribución especificada $F_0(x)$.
    -   $H_i:$ Los datos observados no provienen de la distribución especificada $F_0(x)$.
...
* En Excel hay dos formas de hacerlo (fórmulas en *inglés*)
  ...
  * Forma larga: 1-CHISQ.DIST($\chi^2$, $k-1$, TRUE), siendo $k$ el número de categorías.
...
* Se ha recolectado 400 observaciones sobre la cantidad de personas que se reciben cada minuto en una estación de tren de la capital. 
...
* Realice una rutina gráfica. 
  * Recomendación: $E_i$ debe sumar 400 (es decir, las probabilidades siempre deben sumar 1), por lo que quizá deba agregar datos en cero, por ejemplo: 
...
* Por lo que estos valores deben agregarse. Para efectos de este curso usaremos $E_i \ge 5$ siempre, como una simplificación, la regla global es: [1)]{.hi} Ningún $E_i < 1$ y [2)]{.hi} al menos el 80% de los $E_i\ge 5$.
...
ejercio_01 |> 
  ...
  rbind(data.frame(Llegadas = ">12", Frecuencia = 0)) |> 
  dplyr::mutate(PEi = c(dpois(0:11, lambda = 4.575), 
                        1- ppois(11, lambda = 4.575)), 
                Ei = PEi*400, 
                chi = (Frecuencia - Ei)^2/Ei) |> 
  janitor::adorn_totals() |> 
  dplyr::mutate(across(2:5, ~round(.x, 2))) |> 
...
* Debido a la forma en que se calcula el valor de $\chi^2$, es [extremadamente sensible al tamaño de la muestra]{.hi}: 
  ...
  * $\chi^2$ es sensible porque con muestras grandes, la potencia del test aumenta tanto que convierte diferencias minúsculas (e irrelevantes en la práctica) en estadísticamente significativas.
...
* Por *tradición* se suele usar `Minitab` para hacerlo, pero se nombrarán otras que no están presentes en este software, pues no es el único que existe ni el que se usa en ambientes fabriles. 
...
* Por lo que la selección de una prueba [debe ser concordante con el tipo de desviación que nos preocupe]{.hi}. Ya que las pruebas, por lo general, suelen darle más importancia a uno de los dos motivos anteriores
...
residuos <- data.frame(X1 =  X1, X2 = X2, y = y) |> 
  dplyr::mutate(y_hat_lineal = drop(cbind(1, X1, X2) %*% coeficientes_regresion),
...
qq <- qqnorm(residuos$residuos_dev, plot.it = FALSE)

# Cuantiles teóricos y observados
qq_data <- data.frame(teorico = qq$x,  observado = qq$y)

# Parámetros de la recta de referencia
qq_line <- data.frame(x = range(qq_data$teorico),
                      y = range(qq_data$teorico))

p1 <- ggplot2::ggplot(qq_data,
                      ggplot2::aes(x = teorico, y = observado)) +
  ggplot2::geom_point() +
  ggplot2::geom_line(data = qq_line,
                     ggplot2::aes(x = x, y = y),
                     color = "#556070") +
  ggplot2::labs(x = "Cuantiles teóricos",
...
# ---- Ahora sí aplicar layout global y tamaño ----
fig <- fig |>
  layout(#height = 780,
         #width  = 850,
